...
namespace MyShop.Areas.MYSHOP.Controllers
{
    public partial class DashboardMenuController : Controller
    {
        ...
        public JsonResult DashboardGridView(FSMDashBoardFilter dd, int userid)
        {
            
            //List<FSMDashboardGridView> dbDashboardData = new List<FSMDashboardGridView>();
            List<FSMDashboardGridView> dashboardGridobj = new List<FSMDashboardGridView>();
            ...
            if (dd.Type == "Attendance")
            {
                ...
                DataTable dt = objgps._GetSalesSummaryReport(DateTime.Now.ToString("yyyy-MM-dd"), Convert.ToString(userid), dd.StateId == "0" ? "" : dd.StateId, "", "", dd.FilterName, dd.BranchId == "0" ? "" : dd.BranchId);
                // End of Mantis Issue 24765

                query = "Select " + ColumnName + " from FTSDASHBOARD_REPORT where USERID='" + Convert.ToString(userid) + "' and ACTION='" + Convert.ToString(dd.FilterName) + "'";
                //query = $"SELECT {ColumnName} FROM FTSDASHBOARD_REPORT WHERE USERID='{userid}' AND ACTION='{dd.FilterName}' {orderby} OFFSET {skip} ROWS FETCH NEXT {take} ROWS ONLY";
                ...
                if (dd.FilterName == "AT_WORK" && dbDashboardData.Rows.Count > 0)
                {
                    foreach (DataRow item in dbDashboardData.Rows)
                    {
                        ...
                        ObjGrid.Department = Convert.ToString(item["Department"]);
                        ObjGrid.ContactNo = Convert.ToString(item["Mobile No."]);
                        ...
                        ObjGrid.OrderValue = Convert.ToString(item["Order Value"]);
                ...
                else if (dd.FilterName == "NOT_LOGIN")
                {
                    foreach (DataRow item in dbDashboardData.Rows)
                    {
                        ...
                        ObjGrid.ContactNo = Convert.ToString(item["Contact No."]);
                        dashboardGridobj.Add(ObjGrid);
                ...
                //if (dd.FilterName == "EMP" && dbDashboardData.Rows.Count > 0)
                {
            ...
            //return Json(dashboardGridobj, JsonRequestBehavior.AllowGet);
            //return Json(dbDashboardData, JsonRequestBehavior.AllowGet);
        ...
        //public PartialViewResult DashboardGridViewFV(FSMDashBoardFilter dd, int userid)
        public JsonResult DashboardGridViewFV(FSMDashBoardFilter dd, int userid)
        {
            List<FSMDashboardGridViewFV> dashboardGridobj = new List<FSMDashboardGridViewFV>();
            ViewBag.WindowSize = dd.WindowSize;
            SalesSummary_Report objgps = new SalesSummary_Report();
            DBEngine objdb = new DBEngine();
            DataTable dbDashboardData = new DataTable();
            ...
            if (dd.Type == "Attendance")
            {
                ...
                dbDashboardData = objdb.GetDataTable(query);
                FSMDashboardGridViewFV ObjGrid;

                if (dd.FilterName == "AT_WORK" && dbDashboardData.Rows.Count > 0)
                {
                    foreach (DataRow item in dbDashboardData.Rows)
                    {
                        ObjGrid = new FSMDashboardGridViewFV();
                        ObjGrid.Employee = Convert.ToString(item["Employee"]);
                        ObjGrid.Designation = Convert.ToString(item["Designation"]);
        ...
        public PartialViewResult DashboardGridViewDetailsTeamVisit(FSMDashBoardFilter dd, int userid)
        {
            ViewBag.WindowSize = dd.WindowSize;
            DataTable dbDashboardData = new DataTable();
        ...
        //public PartialViewResult DashboardGridViewDetails(FSMDashBoardFilter dd, int userid)
        public JsonResult DashboardGridViewDetails(FSMDashBoardFilter dd, int userid)
        {
            List<FSMDashboardGridViewShopDetails> dashboardGridobj = new List<FSMDashboardGridViewShopDetails>();
            ViewBag.WindowSize = dd.WindowSize;
            DataTable dbDashboardData = new DataTable();
            dbDashboardData = dashbrd.GetSalesManVisitDetails(dd.EMPCODE, userid);

            FSMDashboardGridViewShopDetails ObjGrid;

            foreach (DataRow item in dbDashboardData.Rows)
            {
                ObjGrid = new FSMDashboardGridViewShopDetails();
                ...
                ObjGrid.Image = Convert.ToString(item["shopvisit_image"]);
        ...
        public PartialViewResult DashboardGridViewDetailsTeamVisitH(FSMDashBoardFilter dd, int userid)
        {
            ViewBag.WindowSize = dd.WindowSize;
            DataTable dbDashboardData = new DataTable();
            dbDashboardData = dashbrd.GetSalesManVisitDetails(dd.EMPCODE, userid);
